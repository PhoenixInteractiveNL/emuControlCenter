...
class TreeviewData {
	...
	* -------------------------------------------------------------------------
	*/
	public function get_file_data_TEST_META($extension, $search_like=false, $limit=array(), $return_count=true, $order_by="", $language=false, $category=false, $search_ext=false, $show_files_only=true, $toggle_show_doublettes=false)
	{
		$ret = array();
		$ret['data'] = array();
		$ret['count'] = array();
	...
	*
	*/
	public function get_bookmarks($extension, $search_like=false, $limit=array(), $return_count=true, $order_by="", $language=false, $category=false, $search_ext=false, $show_files_only=true, $toggle_show_doublettes=false)
	{
		#$snipplet_extension = ($extension) ? "fd.file_ext='".sqlite_escape_string($extension)."'" : "1";
		
		$ret = array();
		$ret['data'] = array();
		$ret['count'] = array();
		
		if ($search_like) $search_like = "AND ".$search_like." ";
		
		$snipplet_extension = " 1  and fd.duplicate is null";
		...
		#$snipplet_extension .= $this->get_search_ext_snipplet($search_ext);
	...
	*
	*/
	public function get_last_launched($extension, $search_like=false, $limit=array(), $return_count=true, $order_by="", $language=false, $category=false, $search_ext=false, $show_files_only=true, $toggle_show_doublettes=false)
	{
		$ret = array();
		$ret['data'] = array();
		$ret['count'] = array();
		...
		$snipplet_extension = " 1  and fd.duplicate is null";
		// eccident
		$snipplet_extension .= ($extension) ? "AND fd.eccident='".sqlite_escape_string($extension)."'" : "";
		// category
		$snipplet_extension .= ($category !== false && $category != "-1") ? "AND md.category=".$category."" : "" ;
		// language
		$snipplet_language_join = ($language) ? "left join mdata_language AS mdl on md.id=mdl.mdata_id " : "";
	...
	*
	*/
	public function get_media_count_for_eccident($eccident) {
		
		$ret = false;
		$where_snip = ($eccident) ? "and eccident='".sqlite_escape_string($eccident)."'" : ""; 
		
		$q = "SELECT count(*) as cnt FROM fdata where duplicate is null ".$where_snip."";
		#print $q."\n";
		$hdl = $this->db->query($q);
		$ret = $hdl->fetchSingle();
		return $ret;
	...
	*
	*/
	public function get_media_count_for_eccident_search($extension, $language=false, $category=false, $search_ext=false)
	{
		$snipplet_extension = "  ";
		// eccident
		$snipplet_extension .= ($extension) ? " fd.eccident='".sqlite_escape_string($extension)."' and fd.duplicate is null " : "fd.duplicate is null ";
