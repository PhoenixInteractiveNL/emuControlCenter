...
class EccParserMedia {
	...
	*/
	...
	/*
	* �berpr�ft, wie vorgegangen werden muss.
	...
	public function add_file($file_data) {
		...
		if ($this->_file_data) {
			...
			if (!$fd_exists) {
				...
				// UPDATE - �nderungen im File == neue checksumme
				if (
	...
	*/
	...
	/*
	* Bei ge�nderten Inhalt hat die gleiche datei eine
	...
	*/
	...
	/*
	* L�scht Files anhand des Pfades aus der datenbank.
	* Wird ben�tigt, wenn der user ein Verzeichnis gel�scht
	...
	*/
	private function filedata_exists(){
		...
		if ($res = $hdl->fetch(SQLITE_ASSOC)) {
			// Checksummen zur weiteren controlle zur�ckreichen
			return $res;
		}
		return false;
	}
	...
	* valide sind, soll hei�en, ob die Files in der DB noch im Filesystem
	* existieren. Wenn nicht, dann wird der DB-Eintrag gel�scht
	*/
	public function optimize($type=false) {
		
		if (!$type && ($this->_basepath)) {
			// nur files im und unterhalb des basepaths �berpr�fen.
