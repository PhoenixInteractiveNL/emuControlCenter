...
GUISetIcon(@ScriptDir & "\kameleon.ico", "", $KameleonCodeGui) ;Set proper icon for the window.
...
While 1
	...
	Switch $nMsg
		...
		Case $KnopCancel
			Exit
		Case $KnopOK
			IniWrite(@ScriptDir & "\kameleon.code", "kameleon", "code", GUICtrlRead($CodeInputBox))
			Exit
	EndSwitch
